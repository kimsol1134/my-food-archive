...
- [x] Task 1: Flutter 프로젝트 생성 및 패키지 의존성 추가 (Android 플랫폼 포함)
- [x] Task 2: **Android 권한 설정 (AndroidManifest + Photo Picker)**
...
- [x] Task 10: **사진 선택 서비스 및 EXIF 메타데이터 추출 (Android Photo Picker)**
- [~] Task 11: Gemini Vision AI 서비스 연동 (Firebase AI Logic) — 앱 코드 완료, **Firebase 콘솔 구성(flutterfire configure + App Check 디버그 토큰)은 사용자 Google 인증 필요 → 미완**. 미구성 시 AI 호출만 null 폴백되고 나머지 기능은 정상 동작하도록 `main.dart`의 Firebase 초기화를 try/catch로 방어함.
- [x] Task 12: 저장 기능 완성 (Create + Update + searchKeyword 생성)
- [x] Task 13: 삭제 기능 완성
- [x] Task 14: 실시간 키워드 검색 기능
- [x] Task 15: 사진 재선택 기능
- [x] Task 16: 에러 처리, 토스트 메시지 마무리
- [x] Task 17: 전체 통합 테스트 및 UI 폴리시 — `flutter analyze` 0 issues, `flutter test` 통과, `flutter build apk --release` 성공(`build/app/outputs/flutter-apk/app-release.apk`, 디버그 키 서명). 에뮬레이터 UC-01~06 실기 검증 + AI 동작 확인은 Firebase 구성 후 잔여.
