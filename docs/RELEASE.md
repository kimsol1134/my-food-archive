...
- 앱 이름: **맛집 아카이브** (`android/app/src/main/res/values/strings.xml` → 매니페스트 `@string/app_name`).
- 런처 아이콘: 앱 블루(#007AFF) + 흰색 "맛" 적응형 아이콘 (`assets/icon/` 소스 → `flutter_launcher_icons`로 생성).
...
2. 앱 만들기 → 앱 이름 "맛집 아카이브", 언어/무료 여부 설정.
