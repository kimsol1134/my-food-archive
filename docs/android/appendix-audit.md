...
| 시각 안내 | 10% | 6.0 | 0.60 | Android 앱 원본 6장은 사용 가능하지만 기존 Play Console 14장은 타이트 크롭이라 원본 재캡처 필요 |
| **종합** | **100%** |  | **9.27 / 10** | 반올림 **9.3 / 10** |

이전 점검의 4.2/10에서 9.3/10으로 올랐다. 가장 큰 개선은 Android를 처음 접하는 독자가 별도 플랫폼 용어를 배우지 않고도 “계정 → AAB → 내부 테스트 → 실제 기기 → 공개” 순서를 이해하도록 전체 설명을 다시 쓴 점이다.

편집 원고와 온라인 자료의 연결만 따로 보면 출판 준비도는 **9.8/10**이다. 고정 랜딩 페이지, 실스캔 QR, 장별 복귀 지점, 그림 캡션, 원본 캡처 기준이 정리되어 있다. 다만 Play Console 원본 재캡처를 마치기 전에는 시각 자료를 출판 완료로 판정하지 않는다.
...
- 기존 Play Console 타이트 크롭은 출판용으로 사용할 수 없다. 자르기·가림·주석이 없는 브라우저 원본 캡처로 모두 교체해야 한다.
