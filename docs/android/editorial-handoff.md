...
| 17장 첫 방문 | 공개 로드맵 1~6절에서 점검과 비공개 테스트 시작 | 17.5절 |
| 14일 조건 충족 후 | 공개 로드맵 7절에서 프로덕션 접근 신청 | 신청 결과 대기 |
| 18장·승인 대기 중 | 공개 로드맵 9절에서 비공개 테스트 링크 확인 | 18.3절 |
| 18장·프로덕션 접근 승인 완료, 출시 미제출 | 공개 로드맵 8~9절에서 프로덕션 출시 제출과 링크 확인 | 18.3절 |
| 프로덕션 출시 제출 후 앱 심사 중 | 공개 로드맵 9절의 승인·심사 대기 경로 | 18.3절 |
| 앱 심사 승인·정식 공개 완료 | 공개 로드맵 9절의 정식 공개 경로 | 18.3절 |
...
책과 온라인 자료에는 자르기·리샘플링·가림·합성·주석을 하지 않은 원본 캡처를 제출합니다. Play Console 14장은 2026년 7월 13일 전체 브라우저 화면으로 다시 캡처했습니다. 계정 이메일, 다른 앱 이름, 주소와 결제 정보가 보이더라도 촬영 원본은 그대로 전달합니다. 편집자는 원본 파일을 직접 배치하고 비율을 유지한 채, 최종 지면에서 공개하면 안 되는 개인정보만 가립니다. 편집용 번호는 파일명의 빈 번호와 무관하게 P-1부터 연속으로 사용합니다.
