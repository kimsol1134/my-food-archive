...
Play Console에 비공개 테스트 요구사항이 표시되는 계정은 최소 14일의 테스트를 마쳐야 프로덕션 접근을 신청할 수 있습니다. 아래 표에서 현재 상태에 해당하는 행만 진행합니다.
...
5. [국가/지역] 항목이 보이면 테스트할 국가와 지역을 선택하고 저장합니다.
6. 출시 노트를 입력하고 [검토]로 이동합니다.
7. 오류가 있으면 해결한 뒤 [출시 시작], [변경사항 전송] 또는 현재 화면에 표시된 제출 버튼을 누릅니다.
8. 비공개 테스트 화면에서 출시 상태를 확인합니다.
...
답변을 읽고 실제 기록과 맞는지 확인한 뒤 Play Console에 입력합니다. 접근 승인은 자동이 아니며 추가 테스트를 요청받을 수 있습니다. 책 17.7절을 이미 읽었으므로 신청을 마치면 18장으로 이동합니다. 18장에서는 현재 승인 상태에 따라 이 문서 9절 또는 8~9절을 사용합니다.
...
4. [비공개 테스트에서 승격]을 선택하고, 비공개 테스트에서 설치해 확인한 `versionCode`와 같은지 확인합니다.
...
Play Console에서 [게시 개요] 또는 프로덕션 상태를 확인합니다. 첫 출시는 관리형 게시를 사용할 수 없으므로 승인된 변경사항이 자동으로 게시됩니다. 상태가 [검토 중], [처리 중] 또는 [게시 중]이면 같은 버전을 다시 제출하지 않고 기다립니다. Google Play 링크가 실제 Android폰에서 열릴 때 아래 정식 공개 경로로 이동합니다.
