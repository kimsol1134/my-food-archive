...
|---|---|---|
| 책 17장을 처음 읽는 중 | 1~6절에서 점검과 비공개 테스트 시작 | 책 17.7절 |
...
2. 이메일 목록, CSV 또는 Google Group 중 한 가지 방법으로 테스터를 추가합니다. Google Group을 사용한다면 테스터가 먼저 그룹 가입을 마쳤는지 확인합니다.
3. 피드백을 받을 이메일 또는 URL을 입력합니다.
4. 아래 두 경로 중 하나만 선택해 비공개 테스트 버전을 준비합니다.
5. 참여 링크와 확인할 기능을 테스터에게 보냅니다.

| 현재 준비 상태 | 진행 방법 | 완료 기준 |
|---|---|---|
| 내부 테스트에서 확인한 버전을 그대로 사용 | 내부 테스트 버전의 [트랙으로 승격] 또는 비슷한 버튼에서 [비공개 테스트] 선택 | 비공개 테스트 트랙에 같은 `versionCode`가 표시됨 |
| 앱을 수정해 새 AAB가 있음 | 비공개 테스트에서 [새 버전 만들기]를 눌러 새 AAB 업로드 | 비공개 테스트 트랙에 새 `versionCode`가 표시됨 |
...
날짜를 직접 계산해 종료하지 않고 Play Console에 신청 가능 상태가 표시될 때까지 참여를 유지합니다.
...
1. 내부 테스트 참여 페이지에서 [프로그램 나가기] 또는 [테스트 종료]를 누릅니다.
2. Play Store가 비공개 테스터로 등록한 Google 계정으로 열려 있는지 확인합니다.
3. 비공개 테스트 참여 링크를 다시 엽니다.
4. Play Console의 비공개 테스트 화면에 올린 버전이 표시되고, Android폰에서 [설치] 또는 [업데이트]가 보이는지 확인합니다.
...
3. [새 버전 만들기] 또는 [새 출시 만들기]를 누릅니다.
4. [비공개 테스트에서 승격]을 선택합니다. 새 AAB를 올리는 경우에는 클로드 코드가 `versionCode`를 올려 다시 빌드하게 합니다.
5. 출시 노트를 입력하고 [검토]로 이동합니다.
6. 화면의 오류와 경고를 클로드 코드에게 보여 주고 필요한 항목을 처리합니다.
7. 첫 출시라면 출시 비율을 100%로 확인하고 [검토를 위해 변경사항 전송] 또는 현재 화면의 제출 버튼을 누릅니다.
...
### 앱 심사가 승인됐지만 링크가 아직 열리지 않는 경우

Play Console에서 [게시 개요] 또는 프로덕션 상태를 확인합니다. 관리형 게시를 켰다면 게시 버튼을 눌러야 할 수 있습니다. 상태가 [게시 중]이면 같은 버전을 다시 제출하지 않고 기다립니다. Google Play 링크가 실제 Android폰에서 열릴 때 아래 정식 공개 경로로 이동합니다.
...
**비공개 테스터에게 보내는 메시지 예시**

```text
마이 맛집 아카이브 Android 테스트 버전을 확인해 줘.
