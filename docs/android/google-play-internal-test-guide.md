...
계정 확인을 기다려야 하면 창을 닫고, 확인 완료 메일이나 완료 상태가 표시된 뒤 5절부터 다시 시작합니다. 내부 테스트 버전의 반영을 기다려야 하면 반영된 뒤 9절부터 다시 시작합니다. 17장부터는 [Android 공개 가이드](release-roadmap.md)로 이동합니다.
...
이 파일을 다음 단계에서 Play Console에 올립니다. 6.1절에서 설정이 바뀐 뒤 만들어진 최신 AAB인지 파일의 생성 시간을 확인합니다.
...
강제 적용을 켠 뒤 같은 설치본에서 사진을 다시 골라 자동 입력이 성공하면 이 단계가 끝납니다. 실패하면 강제 적용을 임의로 끄지 않고 오류 화면이나 문장을 클로드 코드에게 보내 원인을 확인합니다.
