...
| Play Console에서 앱 만들기 | 업로드 키와 release 서명 설정 안내 |
...
| 개인정보처리방침 URL | 내부 테스트만 할 때는 당장 막히지 않을 수 있지만, 비공개 테스트, 공개 테스트, 정식 공개로 넘어가려면 준비해야 합니다. |
...
3. 업로드 키가 없으면 새로 만드는 방법을 안내해줘.
4. keystore, key.properties, 비밀번호 같은 민감한 값은 GitHub에 올라가지 않게 .gitignore를 확인해줘.
5. Google Play 제출에 필요한 targetSdk 기준을 만족하는지 확인해줘.
6. 내가 직접 입력해야 하는 비밀번호나 계정 정보는 화면에서 멈춰서 알려줘.
...
2026년 5월 28일 기준 Google Play에 새 앱이나 앱 업데이트를 제출하려면 Android 15, API level 35 이상을 target해야 합니다. 이 값은 매년 바뀔 수 있으니, 빌드 전에 클로드 코드에게 최신 target SDK 요구사항과 프로젝트의 `compileSdk`, `targetSdk` 값을 함께 확인하게 합니다.
...
이건 앱 문제가 아니라 Java 경로 설정 문제일 때가 많습니다. 이때는 터미널에서 계속 버티기보다 Android Studio 화면으로 우회해도 됩니다.

```text
Windows에서 keytool 명령어가 없다는 오류가 나왔어.
터미널 대신 Android Studio 메뉴에서 업로드 키를 만드는 방법으로 안내해줘.
Build > Generate Signed Bundle/APK 흐름을 초보자 기준으로 설명해줘.
```

클로드 코드는 보통 아래 일을 확인합니다.

- `pubspec.yaml`의 `version` 값 확인
- Android 패키지 이름 확인
- release 서명 설정 확인
- 업로드 키가 없으면 만드는 방법 안내
...
테스터 목록을 저장하면 **참여 링크** 또는 **opt-in 링크**가 표시됩니다. 이 링크를 복사합니다.
...
| 앱 아이콘 | 512 x 512 PNG |
| 기능 그래픽 | Google Play 상단에 쓰이는 대표 이미지 |
