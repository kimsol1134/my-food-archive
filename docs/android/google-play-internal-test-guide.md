...
| 책 17장 | 11절~12.3절 (실제 작업은 12절 항목을 먼저 채운 뒤 11절) | 책 17.5절의 점검 프롬프트로 마무리 |
| 책 18장 | 12.4절 | 책 18.4절로 돌아가기 |
...
3. 업로드 키가 없으면 나에게 비밀번호만 물어보고, 키 생성부터 release 서명 설정 연결까지 네가 직접 해줘.
...
6. 비밀번호 외에 내가 직접 입력해야 하는 것이 있으면 멈춰서 알려줘.
7. 마지막에는 Play Console에 올릴 .aab 파일 경로를 알려줘.
```

여기서 사람이 정할 것은 사실상 하나입니다. 업로드 키의 **비밀번호**입니다.

Google 계정 비밀번호를 쓰면 안 됩니다. 이 열쇠 전용의 새 비밀번호를 만드세요. 잊어버리면 재설정 절차를 밟아야 하니, 평소 쓰는 비밀번호 보관 방법에 적어 둡니다.

클로드 코드에게 알려주는 것이 걱정될 수 있습니다. 괜찮습니다. 이 비밀번호는 내 컴퓨터의 `key.properties` 파일에만 저장됩니다. GitHub에는 올라가지 않습니다. 위 프롬프트의 조건 4가 바로 그 확인입니다.
...
2026년 5월 28일 기준으로 Google Play에 새 앱이나 업데이트를 제출하려면 API level 35 이상을 target해야 합니다. 이 기준은 매년 올라갑니다. 그래서 빌드 전에 클로드 코드에게 최신 요구사항과 프로젝트의 `compileSdk`, `targetSdk` 값을 함께 확인하게 합니다. 기준에 못 미치면 직접 고치게 하면 됩니다. 사람이 설정 파일을 열 필요는 없습니다.
...
- Android 패키지 이름 확인
- 업로드 키가 없으면 비밀번호를 물어본 뒤 `keytool`로 직접 생성
...
```text
방금 빌드한 .aab 파일이 있는 폴더를 파인더로 열어줘.
```

Windows에서는 파인더 대신 탐색기라고 하면 됩니다. 열린 창에서 `app-release.aab` 파일을 Play Console의 업로드 영역으로 끌어다 놓습니다.
...
| 대상 API 수준 경고 | Google Play 요구 target SDK보다 낮을 수 있습니다. | 2026년 5월 28일 기준 새 앱/업데이트는 Android 15, API 35 이상을 target해야 합니다. 공식 요구사항을 다시 확인하고 Android 빌드 설정을 올립니다. |
...
> 17장에서 이 절로 왔다면, 읽기는 이 절부터 시작하되 실제 작업은 12절의 앱 콘텐츠와 스토어 등록정보를 먼저 채우는 편이 자연스럽습니다. 그 항목들이 채워져야 비공개 테스트 출시 버튼이 열립니다.
...
이 절은 내부 테스트만 할 때는 오늘 당장 전부 하지 않아도 됩니다. 다만 비공개 테스트나 Google Play 정식 공개까지 갈 생각이라면 준비해야 합니다.
...
**여기까지가 17장 대응입니다.** 제출 전 마지막 점검은 책 17.5절의 프롬프트를 쓰면 됩니다. 17.5절에 Google Play 기준으로 바꾼 Android용 문장이 그대로 실려 있습니다. 비공개 테스트 14일을 채우고 프로덕션 접근이 승인되면 12.4절로 이어 갑니다.
