...
created: 2026-05-28
updated: 2026-07-12
source_chapter: "15~18장 Android 대응 (TestFlight/App Store → Google Play)"
---
...
15장의 TestFlight에 가장 가까운 Google Play 기능은 **내부 테스트**입니다. 아직 Google Play에 공개하지 않은 앱을 내 Android 기기와 몇 명의 테스터에게 먼저 설치해 보는 통로입니다.

이 자료의 목표는 정식 출시가 아닙니다. 목표는 하나입니다.

**14장까지 만든 Android 앱을 Google Play Console의 내부 테스트에 올리고, 실제 Android 기기에서 설치해 보는 것.**

정식 공개, 프로덕션 접근 신청, Google Play 링크 공개는 다음 단계입니다. 다만 새 개인 개발자 계정에서는 비공개 테스트 12명/14일 요구사항이 실제로 큰 변수입니다. 이 자료에서는 내부 테스트를 먼저 마친 뒤, **비공개 테스트 요구사항을 현실적으로 준비하는 방법**까지 함께 짚겠습니다.

지금 어느 장을 읽고 있는지에 따라 진행할 절이 다릅니다.

| 지금 읽는 곳 | 이 자료에서 진행할 절 | 끝나면 |
|---|---|---|
...
> 화면을 캡처해서 클로드 코드나 다른 사람에게 공유할 때는 Google 계정 이메일, 실명, 주소, 전화번호, 결제 정보, 테스터 이메일, 참여 링크 전체가 보이지 않게 가립니다.

---

## 0. TestFlight와 Google Play 내부 테스트는 이렇게 대응됩니다

15장을 읽은 뒤 이 자료를 보면 아래처럼 대응됩니다.

| 책 15장 iPhone 흐름 | Android 온라인 자료 흐름 |
|---|---|
| Apple Developer Program 가입 | Google Play Console 개발자 계정 만들기 |
| App Store Connect에 앱 등록 | Play Console에 앱 만들기 |
| Xcode 코드 서명 / API Key 준비 | Android 업로드 키 / 앱 번들 서명 준비 |
| TestFlight에 빌드 업로드 | 내부 테스트 또는 비공개 테스트 트랙에 Android App Bundle 업로드 |
| TestFlight 내부 테스트 그룹 | Google Play 내부 테스트 |
| TestFlight 외부 테스트 그룹 | Google Play 비공개 테스트 |
| TestFlight 초대 메일/공개 링크 | Google Play 테스터 이메일 목록, Google Group, 참여 링크 |
| 내 아이폰에 설치 | 실제 Android 기기에 Google Play로 설치 |

중요한 차이도 있습니다.

Apple 쪽은 TestFlight와 App Store Connect가 한 화면 안에서 이어집니다. Google Play 쪽은 **앱 번들 서명**, **테스터 목록**, **앱 콘텐츠**, **스토어 등록정보**, **비공개 테스트 요구사항**이 더 잘게 나뉩니다.

그래서 Android 자료는 15장보다 안내판이 조금 더 많습니다. 대신 원리는 같습니다. 클로드 코드가 할 수 있는 일은 맡기고, 계정 확인과 콘솔 클릭처럼 사람이 해야 하는 일만 우리가 처리합니다.

---

## 1. 전체 흐름

이번 절에서는 실제로 누를 순서만 먼저 정리합니다. 오늘 할 일은 일곱 단계입니다.
...
정식 공개와 비공개 테스트 준비는 오늘 하지 않습니다.

Google Play는 개인 개발자 계정의 경우 정식 공개 전에 비공개 테스트 12명/14일 요구사항이 있습니다. 그 준비는 16장에서 앱을 다듬은 뒤, 이 자료의 11절에서 시작합니다. 오늘은 내부 테스트로 앱을 내 폰에 설치해 보는 것까지입니다.

이번 자료에서도 책 15장과 같은 방식으로 진행합니다. 사람이 해야 하는 일과 클로드 코드가 할 수 있는 일을 나눕니다.

| 사람이 직접 해야 하는 일 | 클로드 코드에게 맡길 수 있는 일 |
|---|---|
| Play Console 개발자 계정 가입 | 프로젝트의 Android 설정 점검 |
| 결제, 본인 확인, 기기 확인 | 패키지 이름과 버전 번호 확인 |
| Play Console에서 앱 만들기 | 업로드 키 생성과 release 서명 설정 |
...
```text
Google Play Console의 이 화면에서 다음에 무엇을 눌러야 할지 모르겠어.
첨부한 캡처를 보고 알려줘.
...
| 개인정보처리방침 URL | 내부 테스트만 할 때는 당장 막히지 않을 수 있지만, 비공개 테스트, 공개 테스트, 정식 공개로 넘어가려면 준비해야 합니다. 만드는 방법은 12.1절에 있습니다. 지금은 없어도 됩니다. |
...
## 3. Google Play Console 개발자 계정 만들기
...
본인 확인은 신청 즉시 끝나지 않을 수 있습니다. 며칠 걸리기도 합니다. 내가 무엇을 잘못해서가 아니라 원래 그런 절차입니다. 기다리는 동안 5절로 넘어가 앱 번들을 먼저 만들어 두면 시간이 아깝지 않습니다. 계정 확인과 빌드는 서로를 기다리지 않아도 됩니다.

가입 중 막히는 화면이 나오면 화면을 캡처해서 클로드 코드에게 묻습니다. 다만 결제 정보와 주소는 가리고 보냅니다.
...
개발자 계정이 만들어지면 Play Console 홈으로 들어갈 수 있습니다. 여기까지가 Apple Developer Program 가입에 해당하는 단계입니다.

![Google Play Console 개발자 계정 시작 화면](../book-screenshots/google-play/g-01-play-console-signup.png)

본인 확인 단계에서는 아래처럼 계정 확인 항목을 차례로 완료합니다.

![Google Play Console 본인 확인 화면](../book-screenshots/google-play/g-02-account-verification.png)

---

## 4. Play Console에 새 앱 만들기

이제 Play Console에 앱을 하나 만듭니다.

Play Console 홈에서 **앱 만들기**를 누릅니다.

![Google Play Console 앱 만들기 화면](../book-screenshots/google-play/g-03-create-app-button.png)
...
![새 앱의 Google Play Console 대시보드](../book-screenshots/google-play/g-04-new-app-dashboard.png)

대시보드에는 해야 할 일이 많이 보일 수 있습니다.

스토어 등록정보, 앱 콘텐츠, 내부 테스트, 앱 서명, 데이터 안전, 콘텐츠 등급 같은 메뉴가 한꺼번에 나오기 때문입니다. 오늘은 그중 내부 테스트에 필요한 흐름만 사용합니다.
...
## 5. 클로드 코드에게 Google Play용 앱 번들을 만들게 하기
...
저도 처음에는 이 대목에서 멈칫했습니다. 키스토어, 업로드 키, 서명처럼 단어마다 무게가 있어 보였습니다. 그런데 막상 해 보니 제가 직접 한 일은 비밀번호 하나를 정한 것뿐이었습니다. 나머지는 클로드 코드가 처리했습니다.

클로드 코드에게 Google Play 내부 테스트에 올릴 앱 번들을 만들어 달라고 부탁합니다.

```text
Google Play Console 내부 테스트에 올릴 Android App Bundle을 만들어줘.

조건은 다음과 같아.
1. Android 앱 기준으로 빌드해줘.
2. 패키지 이름과 versionCode/versionName을 먼저 확인해줘.
3. 업로드 키가 없으면 keytool을 실행한 뒤, 비밀번호 입력 프롬프트에서 내가 직접 입력하도록 멈춰줘. 비밀번호를 채팅이나 로그에 출력하지 마.
4. keystore, key.properties, 비밀번호 같은 민감한 값은 GitHub에 올라가지 않게 .gitignore를 확인해줘.
5. Google Play 제출에 필요한 targetSdk 기준을 만족하는지 확인하고, 부족하면 직접 고쳐줘.
6. 비밀번호나 계정 인증처럼 내가 직접 입력해야 하는 것이 있으면 멈춰서 알려줘.
7. 마지막에는 Play Console에 올릴 .aab 파일 경로를 알려줘.
```
...
Windows와 Mac 모두 빌드 전에 아래를 한 번 확인합니다.
...
1. 내 컴퓨터에서 keytool 위치를 찾아줘. Android Studio 설치 폴더의 jbr 안에 있는 경우가 많아.
2. 찾으면 전체 경로로 네가 직접 실행해서 업로드 키를 만들어줘.
3. 그래도 안 되면 Android Studio 메뉴(Build > Generate Signed Bundle/APK)에서
   만드는 방법을 초보자 기준으로 안내해줘.
```
...
![AAB 빌드 완료와 결과 파일 경로](../book-screenshots/google-play/g-05-aab-build-result.png)
...
공식 안내 기준으로 내부 테스트는 앱 하나당 최대 100명의 테스터까지 빠르게 배포할 수 있습니다. 앱 설정이 아직 완전히 끝나지 않아도 내부 테스트는 시작할 수 있습니다. 그래서 15장의 TestFlight 첫 업로드에 가장 가까운 단계입니다.

내부 테스트 화면에서 **새 버전 만들기** 또는 **새 출시 만들기**를 누릅니다.

![Google Play Console 내부 테스트 화면](../book-screenshots/google-play/g-06-internal-testing.png)

### 6.1 Play App Signing 안내 확인
...
![Play App Signing 안내 화면](../book-screenshots/google-play/g-07-play-app-signing.png)
...
열린 창에서 `app-release.aab` 파일을 Play Console의 업로드 영역으로 끌어다 놓습니다.

![Android App Bundle 업로드 화면](../book-screenshots/google-play/g-08-upload-aab.png)
...
![내부 테스트 출시 검토 화면](../book-screenshots/google-play/g-09-review-release.png)
...
![내부 테스트 참여 링크 화면](../book-screenshots/google-play/g-11-opt-in-link.png)
...
![테스터 참여 페이지](../book-screenshots/google-play/g-12-opt-in-page.png)
...
### 8.1 수정한 앱을 다시 내부 테스트에 올리는 절차

> 이 절은 16장을 진행할 때 사용합니다. 지금 15장을 따라 처음 읽는 중이라면 건너뛰고 9절로 가면 됩니다.
...
15장에서 TestFlight 빌드를 바로 외부 사람에게 보내기보다 내 아이폰에서 먼저 확인했던 것처럼, Android도 먼저 내 기기에서 확인합니다.
...
**여기까지가 15장 대응입니다. 책 15.5절로 돌아가세요.**

11절부터는 정식 공개를 준비하는 흐름입니다. 16장에서 아이콘과 첫인상을 다듬은 뒤에 진행하는 편이 좋습니다. 비공개 테스트 14일은 한번 시작하면 유지해야 하는 시계라서, 보여 줄 준비가 된 앱으로 시작하는 편이 안전합니다.

---

## 11. 비공개 테스트 12명/14일을 준비하기

> 17~18장 독자는 실제 작업 순서로 다시 정리한 [`release-roadmap.md`](release-roadmap.md)를 우선 사용하세요. 아래 11~12절은 테스터 모집 문구와 세부 설명이 더 필요할 때 찾아보는 참고 자료입니다.
...
여기서 한 가지를 조심해야 합니다. 내부 테스트는 앱 설정이 덜 끝난 상태에서도 빠르게 시작할 수 있지만, 비공개 테스트는 앱 설정을 마친 뒤 시작하는 흐름입니다. 비공개 테스트에 들어가기 전에는 12절의 앱 콘텐츠, 스토어 등록정보, 개인정보처리방침 URL, 데이터 안전 항목 작성을 완료해야 합니다. 이 항목들이 채워지지 않으면 비공개 테스트 출시 버튼이 막힐 수 있습니다.
...
## 12. 비공개 테스트와 정식 공개 전에 반드시 채울 항목

이 절은 내부 테스트만 할 때는 오늘 당장 전부 하지 않아도 됩니다. 비공개 테스트와 정식 공개의 실제 실행 순서는 [`release-roadmap.md`](release-roadmap.md)를 따르고, 여기서는 각 항목의 상세 설명만 참고합니다.
...
개인정보처리방침을 만드는 방법은 책 17.3절과 같습니다. 17.3절은 iPhone 앱 기준이지만, 방침을 쓰고 공개 페이지에 올리는 일 자체는 플랫폼과 무관합니다. 클로드 코드에게 초안을 부탁하고, Notion 같은 공개 페이지에 올린 뒤, 그 주소를 Play Console의 앱 콘텐츠에 있는 개인정보처리방침 칸에 넣습니다.
...
책 17.3절처럼 방침 페이지 하나에 문의 이메일 안내까지 함께 넣어 두면, 지원 주소가 필요한 항목에도 같은 주소를 쓸 수 있습니다.
...
스크린샷은 16장 iPhone 흐름처럼 실제 기기에서 확인한 뒤 고르는 편이 좋습니다. 내부 테스트 설치가 끝나면 실제 Android 기기에서 홈 화면, 입력 화면, 저장 결과, 검색 화면을 캡처합니다.
...
**여기까지가 17장 상세 참고입니다.** 제출 전 마지막 점검은 책 17.5절의 프롬프트와 [`release-roadmap.md`](release-roadmap.md)의 최종 체크리스트를 함께 사용합니다.

### 12.4 프로덕션 트랙에 출시하기 (18장 대응)

프로덕션 접근 신청이 승인되면 드디어 정식 공개를 할 수 있습니다. 책 18.2절에서 iPhone 앱의 출시 버튼을 누르는 장면에 해당하는 단계입니다. 저도 이 버튼 앞에서는 잠깐 심호흡을 했습니다. 막상 누르면 생각보다 담백하게 끝납니다.
...
심사에서 수정 요청을 받아도 끝난 것이 아닙니다. 책 18.1절의 방식 그대로, 수정 요청 내용을 복사해 클로드 코드에게 분류시키고, 고쳐서 다시 올리면 됩니다.
