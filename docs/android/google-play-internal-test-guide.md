...
이 자료는 책 15장을 Android 앱 기준으로 따라가기 위한 온라인 보충 자료입니다.
...
이 자료는 책 15장부터 18장까지에 걸쳐 사용합니다. 지금 어느 장을 읽고 있는지에 따라 진행할 절이 다릅니다.

| 지금 읽는 곳 | 이 자료에서 진행할 절 | 끝나면 |
|---|---|---|
| 책 15장 | 0~10절 | 책 15.5절로 돌아가기 |
| 책 16장 | 8절 설치 확인, 앱을 고친 뒤 8.1절 | 책 16.5절로 돌아가기 |
| 책 17장 | 11절~12.3절 | 책 17.5절의 점검 프롬프트로 마무리 |
...
이번 절에서는 실제로 누를 순서만 먼저 정리합니다. 오늘 할 일은 여덟 단계입니다.
...
8. 비공개 테스트 12명/14일을 준비합니다.

정식 공개는 오늘 하지 않습니다.

Google Play는 특히 개인 개발자 계정의 경우 정식 공개 전에 별도 테스트 요구사항이 있을 수 있습니다. 내부 테스트로 앱을 내 폰에 설치해 본 뒤, 그다음 비공개 테스트와 프로덕션 공개를 준비합니다.
...
Google 계정 비밀번호를 쓰면 안 됩니다. 이 열쇠 전용의 새 비밀번호를 만드세요. 잊어버리면 나중에 재설정 절차를 밟아야 하니, 평소 쓰는 비밀번호 보관 방법에 적어 둡니다. 클로드 코드에게 알려주는 것이 걱정될 수 있는데, 이 비밀번호는 내 컴퓨터의 `key.properties` 파일에만 저장되고 GitHub에는 올라가지 않습니다. 위 프롬프트의 조건 4가 바로 그 확인입니다.
...
Android toolchain에 오류가 나와도 직접 해결하려고 하지 않아도 됩니다. "flutter doctor의 Android toolchain 오류를 해결해줘. 필요한 것은 네가 설치해줘."라고 클로드 코드에게 맡기면, Android SDK 설치와 라이선스 동의까지 터미널에서 처리할 수 있습니다. 빌드 중에 부족한 구성요소가 나와도 라이선스만 동의되어 있으면 자동으로 내려받아 채웁니다.

API level은 Android 버전마다 붙는 기술 번호입니다. 예를 들어 Android 15의 API level은 35입니다. target은 이 앱이 어느 Android 버전 기준으로 만들어졌는지 표시하는 값입니다. 2026년 5월 28일 기준 Google Play에 새 앱이나 앱 업데이트를 제출하려면 Android 15, API level 35 이상을 target해야 합니다. 이 값은 매년 바뀔 수 있으니, 빌드 전에 클로드 코드에게 최신 target SDK 요구사항과 프로젝트의 `compileSdk`, `targetSdk` 값을 함께 확인하게 합니다. 기준에 못 미치면 직접 고치게 하면 됩니다. 사람이 설정 파일을 열 필요는 없습니다.
...
프로덕션 접근 신청이 승인되면 드디어 정식 공개를 할 수 있습니다. 책 18.2절에서 iPhone 앱의 출시 버튼을 누르는 장면에 해당하는 단계입니다.
