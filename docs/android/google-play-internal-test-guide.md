...
Android 온라인 자료는 두 문서로 나뉩니다.

1. **15~16장:** 지금 보고 있는 문서의 1~11절에서 내부 테스트 설치까지 진행합니다.
2. **17~18장:** [`release-roadmap.md`](release-roadmap.md)에서 비공개 테스트부터 정식 공개까지 진행합니다.
...
| 책 17장 | [`release-roadmap.md`](release-roadmap.md)의 1~7절 | 책 17.5절로 돌아가기 |
| 책 18장 | [`release-roadmap.md`](release-roadmap.md)의 8~10절 | 책 18.3절로 돌아가기 |

이 표가 전체 읽기 순서입니다. 15~16장에는 이 문서의 1~11절만 사용하고, 17장부터는 `release-roadmap.md`로 자료를 바꿉니다.

이 문서 뒤쪽의 [테스터 모집 참고 자료](#테스터-모집-참고-자료)와 [Play Console 작성 참고 자료](#play-console-작성-참고-자료)는 순서대로 읽는 본문이 아닙니다. 공개 로드맵 6절에서 테스터를 구하기 어려울 때는 테스터 모집 참고 자료를, Play Console 질문에 답하기 어려울 때는 Play Console 작성 참고 자료에서 필요한 항목만 찾아봅니다. [공식 문서](#공식-문서)는 Google의 최신 기준을 직접 확인해야 할 때만 엽니다.
...
이번 절의 범위는 내부 테스트로 앱을 내 폰에 설치하고 첫 테스터에게 안내하는 것까지입니다. Google Play 개인 개발자 계정의 비공개 테스트 12명/14일 준비는 17장에서 [`release-roadmap.md`](release-roadmap.md)의 6절을 따라 진행합니다.
...
| 개인정보처리방침 URL | 내부 테스트만 할 때는 당장 막히지 않을 수 있지만, 비공개 테스트와 정식 공개로 넘어가려면 준비해야 합니다. 17장에서 공개 로드맵 2절을 따라 준비합니다. 지금은 없어도 됩니다. |
...
이 부분은 공개 로드맵 6절에서 테스터를 직접 구하기 어렵거나 외부 모집 서비스를 검토할 때만 찾아봅니다. 실제 작업 순서는 [`release-roadmap.md`](release-roadmap.md)를 따릅니다.
...
내부 테스트는 앱 설정이 덜 끝난 상태에서도 시작할 수 있지만, 비공개 테스트에 들어가기 전에는 공개 로드맵 1~4절의 앱 콘텐츠, 스토어 등록정보, 개인정보처리방침 URL, 데이터 안전 항목을 완료해야 합니다. 이 항목들이 채워지지 않으면 비공개 테스트 출시 버튼이 막힐 수 있습니다.
...
1. 공개 로드맵 1~4절의 앱 콘텐츠와 스토어 등록정보를 채운다.
...
### 테스터 모집을 외부에 맡길 때의 확인 기준

지인만으로 12명을 모으기 어렵다면 유료 모집 서비스를 이용하거나, 다른 Android 앱 개발자와 서로 테스터가 되는 방법을 사용할 수 있습니다. 어느 방법을 선택해도 [Google의 공식 기준](https://support.google.com/googleplay/android-developer/answer/14151465?hl=ko)은 같습니다.

- 12명 이상이 비공개 테스트에 참여한다.
- 최근 14일 동안 참여 상태를 연속으로 유지한다.
- 앱의 기능을 사용하고 의견을 전달한다.
- 개발자는 받은 의견과 반영한 수정 내용을 기록한다.

테스터가 14일 안에 참여를 취소하면 그 사람은 14일을 채운 테스터로 계산되지 않습니다. 12명만 맞추지 말고 15명 정도를 모집하면 중간 이탈에 대응하기 쉽습니다.

#### 방법 1. 크몽 같은 외주 플랫폼에서 유료로 모집하기

[크몽](https://kmong.com/) 검색창에 아래 표현을 차례로 입력합니다.
...
검색 결과에서 앱 개발 전체 대행이 아니라 **비공개 테스트 참여자를 모집하고 관리하는 서비스**를 찾습니다. 상품 제목보다 최근 구매 후기와 서비스 범위를 먼저 확인합니다.

결제 전에는 판매자에게 아래 내용을 메시지로 확인합니다.

1. 실제 사람이 각자의 Android 기기에서 참여하는가
2. 12명보다 여유 있는 인원을 제공하는가
3. 모든 테스터가 14일 연속으로 참여 상태를 유지하는가
4. 중간에 나간 테스터가 생기면 추가 인원을 투입하거나 기간을 연장하는가
5. 설치 화면뿐 아니라 앱을 사용한 결과와 피드백을 제공하는가
6. 테스트 시작일, 참여 인원, 기기 또는 Android 버전, 피드백을 정리해 주는가
7. 기준을 채우지 못했을 때 재진행 또는 환불 조건이 있는가
8. Play Console 로그인 정보나 소유자 권한 없이 진행할 수 있는가

판매자에게 넘길 정보는 테스트 참여 링크, 앱 이름, 확인할 기능과 피드백을 받을 연락처면 충분합니다. Google 계정 비밀번호, 인증 번호, 앱 서명 키, 결제 정보는 보내지 않습니다. Play Console에 판매자를 초대하지 않고 진행할 수 있는 서비스를 선택합니다.

다음과 같은 상품은 선택하지 않습니다.

- "프로덕션 승인 100% 보장"처럼 Google의 심사 결과까지 보장한다.
- 실제 사용이나 피드백 없이 가입 숫자만 채운다.
- 에뮬레이터, 봇 또는 여러 명이 함께 쓰는 Google 계정을 사용한다.
- 5점 평가, 좋은 공개 리뷰, 설치 수 증가를 묶어서 판매한다.
- 작업 범위, 14일 유지 방법, 이탈 시 대응을 서면으로 알려 주지 않는다.

Google은 허위 또는 보상성 평점·리뷰와 부당한 설치 수 증가를 [정책으로 금지](https://support.google.com/googleplay/android-developer/answer/9898684?hl=ko)합니다. 비용은 테스터의 시간과 피드백에 지급하는 것이며, 별점이나 공개 리뷰를 사는 데 사용하지 않습니다.

#### 방법 2. Android 앱 개발자 커뮤니티에서 품앗이하기

다른 개발자의 앱을 14일 동안 테스트하고, 상대도 내 앱을 테스트하는 방식입니다. Reddit의 [r/AndroidClosedTesting](https://www.reddit.com/r/AndroidClosedTesting/), [r/AndroidAppTesters](https://www.reddit.com/r/AndroidAppTesters/), [r/TestersCommunity](https://www.reddit.com/r/TestersCommunity/)에는 `test for test`, `will test yours back`, `12 testers for 14 days`와 같은 교환 글이 올라옵니다. 국내 개발자 모임에서는 `구글 플레이 비공개 테스트 품앗이` 또는 `안드로이드 12명 14일 테스터 교환`으로 검색합니다.

영어 커뮤니티에 올릴 때는 아래 문구에서 앱 이름과 링크만 바꿉니다.
...
I will share my tester link and feedback form there.
```

품앗이는 다음 순서로 진행합니다.

1. 모집 글에 앱의 목적, 확인할 기능, 14일 유지 조건을 적는다.
2. 서로의 테스트 링크와 Google 계정 이메일은 공개 댓글이 아닌 개인 메시지로 주고받는다.
3. 상대 앱에 참여한 뒤 설치 화면을 보내고, 내 앱의 참여 여부도 확인한다.
4. 테스트할 기능을 한두 개로 명확히 정하고 실제 사용 결과를 교환한다.
5. 14일 동안 참여 상태를 유지하고, 중간에 그만둘 때는 상대에게 알린다.
6. 받은 피드백과 수정한 내용을 표로 남긴다.

무료 품앗이라도 한 번에 정확히 12명만 모으면 한 명이 빠졌을 때 다시 모집해야 합니다. 15명 정도를 확보하고, 테스트 시작일을 가능한 한 맞추는 편이 관리하기 쉽습니다. 여러 앱을 동시에 교환할 때는 아래 표를 별도 메모에 만들어 둡니다.

| 테스터 별칭 | 모집 경로 | 참여일 | 확인할 기능 | 받은 피드백 | 14일 유지 |
|---|---|---|---|---|---|
| tester-01 | 커뮤니티 교환 | 7월 15일 | 사진 저장과 검색 | 검색 버튼을 찾기 어려움 | 진행 중 |

Google 계정 이메일은 이 표에 적지 않고 Play Console의 테스터 목록에서만 관리합니다. 공개 게시물이나 공동 문서에 다른 사람의 이메일 주소를 올리지 않습니다.

유료 모집과 품앗이를 함께 사용해도 됩니다. 프로덕션 접근 신청서에는 실제 모집 방법을 그대로 쓰고, 테스터가 사용한 기능, 받은 의견, 수정한 내용을 중심으로 답합니다.

### 의뢰하기 전에 확인할 질문

상품 설명이나 커뮤니티 모집 글을 찾았다면 내용을 그대로 복사해 클로드 코드에게 확인을 맡깁니다.
...
## Play Console 작성 참고 자료

이 부분은 공개 로드맵 1~4절을 진행하면서 Play Console의 질문이나 입력 항목을 이해하기 어려울 때만 찾아봅니다. 비공개 테스트와 정식 공개의 실제 실행 순서는 [`release-roadmap.md`](release-roadmap.md)를 따릅니다.
