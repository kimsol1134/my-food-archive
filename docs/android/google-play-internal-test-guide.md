...
이번 절에서는 실제로 진행할 순서 여덟 단계를 먼저 정리합니다.
...
새 Play 앱을 만들면 입력한 패키지 이름이 개발자 계정에 자동으로 등록됩니다. [Android 개발자 인증]에서 새 앱의 등록 상태를 확인하고, 클로드 코드에게 프로젝트의 패키지 이름과 같은지 비교하게 합니다.
...
> 패키지 이름은 앱을 만들 때 등록되며 이후 바꾸기 어렵습니다.
>
> 예를 들어 `com.solkim.my_food_archive`로 앱을 만들면 나중에 같은 앱을 `com.example.myfood`처럼 바꿀 수 없습니다. 앱 이름은 바꿀 수 있지만 패키지 이름은 앱을 구분하는 고유 ID이므로 [앱 만들기]를 완료하기 전에 클로드 코드에게 프로젝트 값과 비교하게 합니다.
...
3. Play Console의 [앱 무결성] → [Play Integrity API]에서 [Cloud 프로젝트 연결]을 누르고 같은 Firebase 프로젝트를 선택합니다.
4. Firebase App Check에서 앱 서명 SHA-256으로 Android 앱의 Play Integrity 제공자를 등록합니다.
5. Firebase App Check의 Android 앱 제공자가 Play Integrity인지 확인합니다.
