...
계정 확인을 기다려야 하면 창을 닫고 나중에 5절부터 다시 시작합니다. 내부 테스트 버전의 반영을 기다려야 하면 9절부터 다시 시작합니다. 17장부터는 [Android 공개 가이드](release-roadmap.md)로 이동합니다.
...
이번 절의 범위는 내부 테스트로 앱을 내 폰에 설치하고 테스터에게 보낼 문장을 준비하는 것까지입니다. Google Play 개인 개발자 계정의 비공개 테스트 12명/14일 준비는 17장에서 [`release-roadmap.md`](release-roadmap.md)의 6절을 따라 진행합니다.
...
#### 첫 업로드 전에 Firebase Android 설정 확인하기

Play App Signing의 앱 서명 정보는 첫 AAB를 올린 뒤에 확인할 수 있습니다. 지금은 업로드 전에 준비할 프로젝트 설정만 클로드 코드에게 맡깁니다.

```text
첫 Android App Bundle을 올리기 전에 Firebase Android 등록과 프로젝트 설정을 확인해줘.
네가 고칠 수 있는 것은 직접 고치고, Firebase Console에서 내가 해야 할 클릭만 알려줘.
Play App Signing 앱 서명 SHA-256은 첫 업로드 뒤에 확인할 거야.
```

클로드 코드는 Android 패키지 이름, Firebase 설정 파일, Google 서비스 연결, Debug와 Release의 App Check 제공자 설정을 확인합니다. 사람은 Firebase Console에서 앱 등록이나 파일 다운로드가 필요할 때만 안내받은 화면을 처리합니다.
...
업로드 키나 디버그 키의 SHA-256을 대신 등록하지 않습니다. 어느 값을 복사해야 할지 모르겠다면 인증서 지문은 보내지 않고 화면에 보이는 항목 이름만 클로드 코드에게 전달합니다.
...
설정 변경 뒤 새 빌드가 필요하면 클로드 코드가 `versionCode`를 올려 AAB를 다시 만들게 합니다. 새 AAB가 만들어지면 내부 테스트의 [새 버전 만들기]에서 업로드하고 출시한 뒤 8절로 이동합니다. 참여 링크를 만든 다음 Google Play 설치본으로 AI 자동 입력까지 확인합니다.
...
문장을 준비했다면 아직 보내지 않고 책 15.5절로 돌아갑니다. 실제 발송은 16.4절에서 수정본을 다시 확인한 뒤 결정합니다.
