...
Google Play Console의 화면 이름과 요구사항은 자주 바뀝니다. 이 자료의 텍스트와 실제 화면이 다르면 민감정보가 보이지 않는 화면만 원본으로 캡처해 클로드 코드에게 보여 주세요. 계정 이메일, 결제 정보, 주소, 전화번호가 보이는 화면은 캡처하지 않고 오류 문구와 버튼 이름만 텍스트로 전달합니다.
