...
| 구분 | 추천 패키지 | 선택 근거 |
| :--- | :--- | :--- |
| **프레임워크** | `Flutter` (SDK ^3.11.4) | 단일 코드베이스에서 공통 Dart 화면과 서비스를 재사용. |
| **상태 관리** | `Provider` | MVP 규모에서 데이터 흐름을 단순하게 유지하기에 적합. |
...
Android에서는 매니페스트에서 화면 방향을 강제로 제한하지 않고 **OS 기본 센서 회전**을 사용합니다.

- **권장 설정**: `android/app/src/main/AndroidManifest.xml`의 `<activity android:name=".MainActivity">`에 `android:screenOrientation`을 선언하지 않습니다. Android 기본값은 `unspecified`입니다.
- 세로 화면만 지원해야 한다는 별도 제품 요구사항이 생긴 경우에만 `android:screenOrientation="portrait"`를 검토합니다. 현재 MVP에서는 기본값을 유지합니다.

### 4.3.2 디스플레이 주사율 (High Refresh Rate)
Android의 화면 주사율은 OS와 Flutter 엔진이 자동으로 처리하므로 별도 플랫폼 설정을 추가하지 않습니다.
...
### 4.3.3 버전 코드 / 버전 네임
`pubspec.yaml`의 `version: 1.0.0+5`는 Android 빌드에서 다음과 같이 자동으로 사용됩니다.

| pubspec | Android 값 |
|---|---|
| `1.0.0` (build-name) | `versionName = "1.0.0"` (`android/app/build.gradle.kts`의 `flutter.versionName`) |
| `+5` (build-number) | `versionCode = 5` (`flutter.versionCode`) |
...
- Play Console에 새 빌드를 올릴 때마다 `pubspec.yaml`의 `+숫자`를 이전 Android 업로드보다 크게 올립니다. 현재 저장소의 첫 Android 업로드는 `+5`를 사용했습니다.
...
> **요점**: Android 패키지명은 `com.solkim.my_food_archive`입니다. `flutterfire configure` 한 줄을 실행하면 콘솔 등록, 설정 파일 생성, Dart 옵션, Gradle 적용을 한꺼번에 처리합니다. 독자가 `google-services.json`을 직접 옮기는 수동 절차부터 시작하지 않도록 이 자동화 경로를 우선 안내합니다.
...
Android에서 반드시 처리할 예외는 다음과 같습니다.
