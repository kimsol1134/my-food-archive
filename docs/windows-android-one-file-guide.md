...
14장까지는 이 문서로 진행합니다. 15장부터는 이 문서에 연결된 Android 내부 테스트 가이드와 공개 로드맵을 각 단계의 단일 기준으로 사용합니다. 중간에 GitHub에서 기획 문서와 Android 기준 설계 문서를 내려받아 `docs` 폴더에 넣고 Claude Code가 읽게 합니다.
...
- Android 앱에 필요한 설정만 설명하는가

결과가 이상하면 이렇게 묻습니다.

```text
방금 답변이 Android 앱 기준에서 벗어난 것 같아.
다시 docs/TRD_android.md를 기준으로 Android 앱에 필요한 흐름만 요약해줘.
```
...
5. Android 앱에 필요하지 않은 플랫폼 설정은 넣지 말 것.
...
특히 아래를 확인해줘.
1. Android 앱에 필요하지 않은 플랫폼 설정이 섞였는가?
...
이후 모든 Task 프롬프트는 새 대화에서 다음 공통 시작문장을 먼저 붙여 넣습니다. 각 절에는 이 문장 뒤에 이어 붙일 Task 요청만 적습니다.
...
2. Android 앱에 필요하지 않은 플랫폼 설정은 추가하지 않는다.
...
6. Firebase AI Logic의 App Check 적용 상태를 확인한다.
7. 내가 직접 입력해야 하는 값은 placeholder로 두고 멈춰서 알려준다.
8. 구현 후 에뮬레이터 또는 연결된 개발 기기에서 실제 사진 한 장으로 메뉴명/카테고리 자동 채움까지 확인한다.
```
...
- 2026년 7월 이후 AI Logic 설정 마법사는 App Check 적용을 자동으로 켤 수 있습니다. Play App Signing 앱 서명 SHA-256, Cloud 프로젝트 연결, Play Integrity 제공자 등록과 Google Play 설치본 확인은 첫 AAB를 올린 뒤 15장의 Android 내부 테스트 가이드 7.5절에서 진행합니다.
...
## 16. 15~16장: Google Play 내부 테스트

15~16장의 실제 작업은 [Android 내부 테스트 가이드](android/google-play-internal-test-guide.md)를 단일 기준으로 사용합니다.

- 책 15장에서는 1~11절을 진행합니다. 9.1절은 건너뜁니다.
- 책 16.4절에서 앱을 수정한 뒤 9.1절을 다시 엽니다. 가까운 사람에게 보낼 경우 11절의 메시지만 사용합니다.
- 계정 확인이나 테스트 버전 반영을 기다린 경우, 가이드에 표시된 재개 절부터 진행합니다.

QA 로그에는 시작한 절, 기다리게 된 화면, 다시 시작한 절, 실제 Android폰 설치 결과만 기록합니다. 배포 절차를 이 문서에 다시 복사하지 않습니다.

---

## 17. 17~18장: 비공개 테스트와 Google Play 공개

17~18장의 실제 작업은 [Android 공개 로드맵](android/release-roadmap.md)을 단일 기준으로 사용합니다.

승인 전, 출시 미제출, 앱 심사 중, 게시 대기와 정식 공개 상태의 범위는 로드맵 상단 상태표를 그대로 따릅니다. 이 QA 문서에 상태표를 복제하지 않습니다.

테스터를 구하는 방법은 [비공개 테스터 모집 안내](android/tester-recruitment.md)를 사용합니다. QA 로그에는 테스트 시작일, 참여 인원, 받은 피드백, 프로덕션 접근 상태와 최종 링크만 기록합니다.
...
docs/TRD_android.md와 docs/Implement_plan_android.md를 기준으로 작업하고 있는지 점검해줘.
```
